...
export function projectDocument(yamlText: string): Record<string, unknown> {
  ...
  const projectDoc = docs.find((d): d is Record<string, unknown> => {
    ...
    return isRecord(root) && ('dependencies' in root || 'devDependencies' in root);
  });
  if (!projectDoc) {
    throw new Error("pnpm-lock.yaml had no document with a project importers['.'] entry");
  }
  return projectDoc;
}
...
export function packageVersionsFromLockfile(yamlText: string): Set<string> {
  ...
  if (!isRecord(doc.packages)) {
    throw new Error("pnpm-lock.yaml's project document did not have a `packages` map");
  }
...
function publishedAt(name: string, version: string): Date {
  ...
  if (!isRecord(parsed) || typeof parsed[version] !== 'string') {
    throw new Error(`pnpm info ${name} time --json had no timestamp for version ${version}`);
  }
  ...
  try {
    ...
    if (tooNew.length > 0) {
      for (const { pkg, publishedAt } of tooNew) {
        const ageDays = ((now - publishedAt.getTime()) / (24 * 60 * 60 * 1000)).toFixed(1);
        console.log(
    ...
    failUnexpected(`dependency age check crashed for PR #${prNumber}: ${String(error)}`);
  }
}
