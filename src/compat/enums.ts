 ...
 *
 * Every agent-specific enum is defined here as a Zod schema. TypeScript types derive via `z.infer`.
 * Alignment with upstream SDKs is verified by compile-time assertions at the bottom of this file.
 *
 * Each export is a PascalCase identifier used as both the runtime schema and the inferred type
 * (TypeScript declaration merging):
 *
 * Import { PermissionBehavior } from "./enums.ts" // schema import type { PermissionBehavior } from
 * "./enums.ts" // type
 */
...
/**
 * Tool names that appear in OpenCode's `AgentConfig.permission` block. Sourced from
...
/**
 * Claude Code's permission mode values, sourced from `@anthropic-ai/claude-agent-sdk`
 * `PermissionMode` type.
 */
