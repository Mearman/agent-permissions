 ...
 *
 * This file defines `.agents/permissions.json` and `.agents/permissions.local.json`. The compiled
 * JSON Schema (`agent-permissions.schema.json`) is generated via `z.toJSONSchema()` and submitted
 * to SchemaStore for IDE autocomplete/validation.
 *
 ...
 *
 * **Security note**: `autonomous`, `bypassPermissions`, and `dontAsk` are only trusted from
 * personal (`permissions.local.json`) or managed (enterprise) sources — NOT from committed
 * `permissions.json`.
 */
...
export const PermissionTiers = z
  .object({
    /**
     * Tools auto-approved without prompting. Evaluated after deny rules — if a deny matches, allow
     * is never checked.
     */
    ...
    /**
     * Tools always denied — short-circuits before allow and ask. Deny rules from ALL sources are
     * merged and checked first. A deny cannot be overridden by an allow in any other source.
     */
    ...
    /**
     * Directories beyond project root that agents may access. Paths are relative to project root or
     * absolute.
     */
    ...
    /**
     * Default permission mode. Also accepted at the top level. Included here for Claude Code
     * compatibility — Claude Code places defaultMode inside the permissions object.
     */
...
export const Rule = z
  .object({
    ...
    /**
     * Pattern to match against tool input. Same syntax as permission rules. When absent, matches
     * any input for the tool (bare tool rule).
     */
    pattern: z
      .string()
      .optional()
...
export const Delegation = z
  .object({
    ...
    /**
     * Per-agent permission overrides. Keys are agent names or glob patterns matching agent
     * identifiers. Values are inline permission tiers that replace (not merge with) the parent
     * policy for that agent.
     *
     * Maps to OpenCode's per-agent markdown frontmatter and Codex's `apps.<name>.tools` config.
     */
 ...
 *
 * When set, the agent harness should enforce filesystem and network boundaries at the OS level
 * (sandbox, container, namespace, etc). This is orthogonal to permission rules — rules control what
 * the agent _asks_ to do; sandbox controls what the OS _allows_ it to do.
 *
...
export const Sandbox = z
  .object({
    ...
    /**
     * Additional paths the agent may write to (beyond project root). Only meaningful when mode is
     * `workspace-write`.
     */
    ...
    /**
     * Whether the agent may make network requests from within the sandbox. When false, network
     * access is blocked at the OS level. Maps to Codex's `sandbox_workspace_write.network_access`.
     */
...
export const Network = z
  .object({
    /**
     * Whether network access is permitted at all. When false, WebFetch and WebSearch are denied
