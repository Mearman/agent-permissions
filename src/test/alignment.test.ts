/**
 * SDK alignment tests — runtime verification that our schemas match the canonical values from each
 * agent SDK.
 *
 * The compile-time checks live in `enums.ts` (type-level assertions). These tests provide a second
 * layer: if someone runs only the tests (not the typecheck), they still get notified of
 * divergence.
 */
