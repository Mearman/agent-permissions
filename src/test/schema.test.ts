...
void describe('AgentPermissionPolicy', () => {
  ...
  void describe('minimal policy', () => {
    ...
    void it('accepts only defaultMode', () => {
      assert.ok(AgentPermissionPolicy.safeParse({ defaultMode: 'readonly' }).success);
    });

    void it('accepts only env', () => {
      assert.ok(AgentPermissionPolicy.safeParse({ env: { FOO: 'bar' } }).success);
    });
  });
  ...
  void describe('validation failures', () => {
    ...
    void it('rejects negative maxDepth', () => {
      assert.ok(!AgentPermissionPolicy.safeParse({ delegation: { maxDepth: -1 } }).success);
    });

    void it('rejects non-string env values', () => {
      assert.ok(!AgentPermissionPolicy.safeParse({ env: { PORT: 3000 } }).success);
    });

    void it('rejects unknown sandbox mode', () => {
      assert.ok(!AgentPermissionPolicy.safeParse({ sandbox: { mode: 'containerised' } }).success);
    });
  ...
  void describe('sandbox configuration', () => {
    void it('accepts sandbox with mode only', () => {
      assert.ok(AgentPermissionPolicy.safeParse({ sandbox: { mode: 'readonly' } }).success);
    });
  ...
  void describe('network controls', () => {
    void it('accepts network enabled only', () => {
      assert.ok(AgentPermissionPolicy.safeParse({ network: { enabled: false } }).success);
    });
