...
function findRules(rules: Rule[] | undefined, tool: string, tier: string): Rule[] {
  return (rules ?? []).filter((r) => r.tool === tool && r.tier === tier);
}

/** Check if a rule exists matching tool, tier, and optionally pattern. */
function hasRule(rules: Rule[] | undefined, tool: string, tier: string, pattern?: string): boolean {
  return (rules ?? []).some(
    (r) => r.tool === tool && r.tier === tier && (pattern === undefined || r.pattern === pattern)
  );
}
...
function getNetworkDomains(net: CodexProfile['network']): Record<string, 'allow' | 'deny'> {
  assert.ok(net !== undefined, 'Expected network to be defined');
  assert.ok(net.domains !== undefined, 'Expected network.domains to be defined');
  return net.domains;
}
...
void describe('claudeCodeCodec', () => {
  ...
  void describe('round-trip (native → canonical → native)', () => {
    void it('preserves allow/deny/ask/defaultMode through full cycle', () => {
      ...
      assert.equal(hasRule(reDecoded.rules, 'Bash', 'allow', 'git status'), true);
...
void describe('crushCodec', () => {
  void describe('decode (Crush → canonical)', () => {
    ...
    void it('passes through unknown tool names as-is', () => {
      ...
      assert.ok(result.rules !== undefined);
      assert.equal(hasRule(result.rules, 'mcp_context7_get-library-doc', 'allow'), true);
    });
  });
...
void describe('codexCodec', () => {
  void describe('decode (Codex → canonical)', () => {
    ...
    void it('converts network domain rules to WebFetch rules', () => {
      ...
      assert.ok(result.rules !== undefined);
      assert.equal(hasRule(result.rules, 'WebFetch', 'allow', 'domain:api.example.com'), true);
      assert.equal(hasRule(result.rules, 'WebFetch', 'deny', 'domain:evil.com'), true);
    });
  ...
  void describe('round-trip (native → canonical → native)', () => {
    ...
    void it('preserves filesystem granular rules through full cycle', () => {
      ...
      assert.equal(hasRule(canonical.rules, 'Write', 'deny', './etc/config'), true);
      const reEncoded = z.encode(codexCodec, canonical);
    ...
    void it('preserves network domain rules through full cycle', () => {
      ...
      const canonical = codexCodec.decode(native);
      assert.ok(canonical.rules !== undefined);
      assert.equal(hasRule(canonical.rules, 'WebFetch', 'allow', 'domain:api.example.com'), true);
      assert.equal(hasRule(canonical.rules, 'WebFetch', 'deny', 'domain:evil.com'), true);
      const reEncoded = z.encode(codexCodec, canonical);
      const profile = getCodexProfile(reEncoded, 'default');
      const domains = getNetworkDomains(profile.network);
      assert.strictEqual(domains['api.example.com'], 'allow');
...
void describe('kiroCodec', () => {
  void describe('decode (Kiro → canonical)', () => {
    ...
    void it('maps web_fetch trusted/blocked to WebFetch rules', () => {
      ...
      assert.ok(result.rules !== undefined);
      assert.equal(
        hasRule(result.rules, 'WebFetch', 'allow', 'url:.*docs\\.aws\\.amazon\\.com.*'),
        true
      );
      assert.equal(hasRule(result.rules, 'WebFetch', 'deny', 'url:.*pastebin\\.com.*'), true);
    });
    ...
    void it('decodes a full agent config', () => {
      ...
      assert.equal(findRules(result.rules, '@git/git_status', 'allow').length, 1);
    });
  });
  ...
  void describe('encode (canonical → Kiro)', () => {
    ...
    void it('encodes Bash deny rules to shell.deniedCommands', () => {
      ...
      assert.deepStrictEqual(encoded.toolsSettings.shell.deniedCommands, ['\\Arm -rf .*\\z']);
    });
    ...
    void it('encodes Write path rules to write settings', () => {
      ...
      assert.deepStrictEqual(encoded.toolsSettings.write.allowedPaths, ['src/**']);
      assert.deepStrictEqual(encoded.toolsSettings.write.deniedPaths, ['./secrets']);
    });
    ...
    void it('encodes WebFetch domain rules to web_fetch settings', () => {
      ...
      assert.deepStrictEqual(encoded.toolsSettings.web_fetch.blocked, ['\\A.*evil.com.*\\z']);
    });
  ...
  void describe('round-trip (native → canonical → native)', () => {
    ...
    void it('preserves shell commands through full cycle', () => {
      ...
      const canonical = kiroCodec.decode(native);
      assert.ok(canonical.rules !== undefined);
      assert.equal(hasRule(canonical.rules, 'Bash', 'allow', 'git status'), true);
      ...
      assert.deepStrictEqual(reEncoded.toolsSettings.shell.allowedCommands, ['\\Agit status\\z']);
      assert.deepStrictEqual(reEncoded.toolsSettings.shell.deniedCommands, ['\\Arm -rf .*\\z']);
    });
    ...
    void it('preserves write paths through full cycle', () => {
      ...
      assert.deepStrictEqual(reEncoded.toolsSettings.write.allowedPaths?.sort(), [
        'src/**',
        'tests/**',
      ]);
      assert.deepStrictEqual(reEncoded.toolsSettings.write.deniedPaths, ['./secrets']);
    });
    ...
    void it('preserves AWS services through full cycle', () => {
      ...
      const canonical = kiroCodec.decode(native);
      assert.ok(canonical.rules !== undefined);
      assert.equal(hasRule(canonical.rules, 'Aws', 'allow', 'service:s3'), true);
      assert.equal(hasRule(canonical.rules, 'Aws', 'deny', 'service:eks'), true);
      const reEncoded = z.encode(kiroCodec, canonical);
      assert.ok(reEncoded.toolsSettings !== undefined);
      assert.ok(reEncoded.toolsSettings.aws !== undefined);
      assert.deepStrictEqual(reEncoded.toolsSettings.aws.allowedServices?.sort(), ['lambda', 's3']);
      assert.deepStrictEqual(reEncoded.toolsSettings.aws.deniedServices, ['eks']);
    });
    ...
    void it('preserves web_fetch patterns through full cycle', () => {
      ...
      assert.ok(canonical.rules !== undefined);
      assert.equal(
        hasRule(canonical.rules, 'WebFetch', 'allow', 'url:.*docs\\.aws\\.amazon\\.com.*'),
        true
      );
      const reEncoded = z.encode(kiroCodec, canonical);
      assert.ok(reEncoded.toolsSettings !== undefined);
    ...
    void it('preserves full agent config through cycle', () => {
      ...
      assert.equal(findRules(reDecoded.rules, '@git/git_status', 'allow').length, 1);
