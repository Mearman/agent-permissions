...
import { convert, validate, check, detectFormat, ConvertError } from '../api.ts';
...
void describe('detectFormat', () => {
  void it('detects Claude Code from allow/deny/ask string arrays', () => {
    assert.equal(detectFormat({ allow: ['Read', 'Bash(git status)'] }), 'claude-code');
  ...
  void it('detects canonical from rules[] with {tool, tier} objects', () => {
    assert.equal(detectFormat({ rules: [{ tool: 'Bash', tier: 'allow' }] }), 'canonical');
  });
  ...
  void it('detects canonical from permissions.allow/deny', () => {
    assert.equal(detectFormat({ permissions: { allow: ['Read'] } }), 'canonical');
  });
...
void describe('convert', () => {
  ...
  void it('converts canonical to crush with explicit from', () => {
    ...
    assert.ok(Array.isArray((result.output as Record<string, unknown>).allowed_tools));
  });
  ...
  void it('throws TypeError on unknown from format', () => {
    assert.throws(() => convert('phantom' as Format, 'canonical', {}), TypeError);
  });

  void it('throws TypeError on unknown to format', () => {
    assert.throws(() => convert('canonical', 'phantom' as Format, {}), TypeError);
  });
