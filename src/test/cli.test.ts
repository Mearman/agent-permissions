...
void describe('CLI', () => {
  ...
  void describe('convert', () => {
    ...
    void it('writes to a file with --output', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'cli-test-'));
      dirs.push(cwd);
      await writeFile(join(cwd, 'input.json'), JSON.stringify({ allow: ['Read'] }));
    ...
    void it('rejects unknown --to format', async () => {
      const result = await run(['convert', '--from', 'canonical', '--to', 'phantom']);
      assert.equal(result.exitCode, 1);
    ...
    void it('auto-detects format from file content', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'cli-test-'));
      dirs.push(cwd);
      await writeFile(join(cwd, 'settings.json'), JSON.stringify({ allow: ['Read'] }));
      const result = await run([
        'convert',
        '--from',
        join(cwd, 'settings.json'),
    ...
    void it('accepts --input as alias for --from', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'cli-test-'));
      dirs.push(cwd);
      await writeFile(join(cwd, 'policy.json'), JSON.stringify({ allow: ['Read'] }));
      const result = await run([
        'convert',
        '--input',
        join(cwd, 'policy.json'),
  ...
  void describe('validate', () => {
    void it('validates a correct policy file', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'cli-test-'));
      dirs.push(cwd);
      await writeFile(join(cwd, 'policy.json'), JSON.stringify({ defaultMode: 'standard' }));
      const result = await run(['validate', '--input', join(cwd, 'policy.json')]);
      assert.equal(result.exitCode, 0);
    ...
    void it('rejects an invalid policy file', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'cli-test-'));
      dirs.push(cwd);
      await writeFile(join(cwd, 'bad.json'), JSON.stringify({ defaultMode: 'yolo' }));
    ...
    void it('rejects invalid JSON', async () => {
      ...
      const result = await run(['validate', '--input', join(cwd, 'broken.json')]);
      assert.equal(result.exitCode, 1);
      assert.match(result.stderr, /invalid JSON/);
    });
    ...
    void it('shows validation error details', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'cli-test-'));
      dirs.push(cwd);
      await writeFile(join(cwd, 'bad.json'), JSON.stringify({ rules: [{ notTool: 'Bash' }] }));
      const result = await run(['validate', '--input', join(cwd, 'bad.json')]);
      assert.equal(result.exitCode, 2);
      assert.match(result.stderr, /rules\./);
  ...
  void describe('sync', () => {
    ...
    void it('syncs a single canonical file (no-op)', async () => {
      ...
      await writeFile(
        ...
        JSON.stringify({ $schema: schemaUrl, defaultMode: 'standard' }, null, 2) + '\n'
      );
    ...
    void it('merges canonical and claude-code configs', async () => {
      ...
      const result = await run(['sync', '--working-dir', cwd, '--yes', '--verbose']);
      assert.equal(result.exitCode, 0);
    ...
    void it('rejects --with and --without together', async () => {
      const result = await run(['sync', '--with', 'claude-code', '--without', 'opencode']);
      assert.equal(result.exitCode, 1);
    ...
    void it('accepts --up numeric value', async () => {
      ...
      const result = await run(['sync', '--working-dir', cwd, '--up', '0', '--yes']);
      assert.equal(result.exitCode, 0);
    });
    ...
    void it('writes backup files with --backup', async () => {
      ...
      const result = await run(['sync', '--working-dir', cwd, '--yes', '--backup']);
      assert.equal(result.exitCode, 0);
      // .bak file should exist for the claude settings
      const { stat } = await import('node:fs/promises');
      await assert.doesNotReject(() => stat(join(cwd, '.claude', 'settings.json.bak')));
    });
    ...
    void it('creates missing config files with --create', async () => {
      ...
      const content = await readFile(join(cwd, '.claude', 'settings.json'), 'utf-8');
