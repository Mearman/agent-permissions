...
void describe('audit report guards', () => {
  ...
  void it('rejects a report whose advisories are not all well-formed', () => {
    assert.equal(isAuditReport({ advisories: { a: advisory(), b: 42 } }), false);
...
void describe('override bookkeeping', () => {
  ...
  void it('withOverrides preserves comments elsewhere in the document', () => {
    const workspace = parseDocument('# a load-bearing comment\nminimumReleaseAge: 10080\n');
  ...
  void it('withOverrides deletes the key entirely for an empty map, rather than writing an empty one', () => {
    const workspace = parseDocument('overrides:\n  a: "1"\nminimumReleaseAge: 10080\n');
...
void describe('classifyAdvisories', () => {
  ...
  void it('defers advisories with no patched version', () => {
    ...
    assert.equal(
      deferred.every((d) => d.reason === 'no patched version exists upstream yet'),
      true
    );
  });
...
`;
  ...
  void it('flags overrides whose selector matches no resolved version, ignoring out-of-selector resolutions', () => {
    ...
    assert.deepEqual(inert, ['undici@<6.27.0', 'undici@>=7.0.0 <7.29.0', 'undici@>=8.0.0']);
  });
});
