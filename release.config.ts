...
/**
 * Single source of truth for the conventional-commit types this project uses. commitlint's allowed
 * type-enum (commitlint.config.ts imports this) and both the commit-analyzer's releaseRules and the
 * release-notes generator's per-type sections below derive from it, so a type can't trigger a
 * release without also being accepted by commit-msg validation, or the reverse.
 *
 * Defined here rather than in a sibling module: semantic-release loads this file via cosmiconfig,
 * which transpiles only this one file, so a separate .ts module would not resolve from it.
